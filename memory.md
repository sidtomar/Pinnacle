...
> **Last updated:** 2026-06-23 (session 3 — incl. 2 code-review passes) | Active branch: `Researchagent_2306` | Main repo: `D:\Codebase\Pinnacle`
...
| 2026-06-23 (session 3) | **Open review items (NOT yet actioned — pending decision):** (a) `pinnacle_pv/index.html` is a ~7,250-line ORPHAN prototype, unreferenced by anything, that even holds a duplicate of the doctor-import feature (`plDb*` vs the portal's `raDb*`, different role gate) — recommend deleting the whole `pinnacle_pv/` dir rather than shipping a second divergent portal. (b) `/admin/upload-doctors` has no auth despite the "Admin" label and does a destructive full-file overwrite — needs a guard before production. (c) `onCLSearchInput` runs `renderCL()` + `clBuildSuggest()` on every keystroke (full re-render per char) — intended live-search UX; a debounce would help on large lists. (d) Non-`DOC`/non-numeric uploaded doctor codes risk id collisions in `get_doctor` (first-match shadowing). |
